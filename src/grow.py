...
def run_training():
    ...
    try:
        ...
        spx_raw = yf.download(
            "^GSPC", period="2y", interval="1h", progress=False, multi_level_index=False
        )
    ...
    log(
        f"Balance de clases — Negativas: {counts[0]} | Positivas: {counts[1]} | Prior: {ratio:.1f}%",
        C.G,
    )
    if ratio < 5.0:
    ...
    model.compile(
        optimizer=tf.keras.optimizers.Adam(1e-3), loss="binary_crossentropy", metrics=["accuracy"]
    )
    ...
    tf2onnx.convert.from_keras(
        model, input_signature=spec, output_path=str(DATA_DIR / "madness.onnx")
    )
    ...
    try:
        ...
        if (
            rknn.load_onnx(
                model=str(DATA_DIR / "madness.onnx"),
                inputs=["input"],
                input_size_list=[[1, TIME_STEPS, len(FEATURES)]],
            )
            == 0
        ):
