...
class DualExchangeManager:
    def __init__(self, testnet=True): # Añadimos bandera de testnet
        ...
        self.safe = getattr(ccxt, os.getenv("XMR_EXCHANGE_ID", "coinex"))({
            "apiKey": os.getenv("XMR_API_KEY"), 
            "secret": os.getenv("XMR_SECRET_KEY")
        })
    ...
    def get_data_batch(self, symbols, limit=500):
        def fetch(s):
            try: return s, self.gen.fetch_ohlcv(s, "1h", limit=limit)
    ...
    def get_data(self, ex, symbol, limit=500):
        try: return ex.fetch_ohlcv(symbol, "1h", limit=limit)
        except: return None
    ...
    def get_sp500_data(self):
        try:
            ...
            spx = yf.download("^GSPC", period="1mo", interval="1h", progress=False, multi_level_index=False, timeout=5)
    ...
    def ensure_bnb_for_fees(self):
        ...
        if self.testnet: return # Saltamos en testnet para evitar errores de API
        try:
    ...
    def bridge_transfer(self, amount, addr, net):
        try: return self.gen.withdraw("USDT", amount, addr, params={"network": net})
        except: return None

    def revisar_log_ordenes(self, symbols=["BTC/USDT", "ETH/USDT", "SOL/USDT", "NEAR/USDT", "PEPE/USDT", "FET/USDT"]):
            ...
            for symbol in symbols:
                try:
                    ...
                    if not orders:
                        ...
                        continue
                    for o in orders:
                        f.write(f"[{timestamp}] 🔹 {symbol} | {o['side'].upper()} | {o['status']} | Cant: {o['amount']} | Precio: {o['price']}\n")
                except Exception as e:
