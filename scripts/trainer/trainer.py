...
def convert_to_rknn(tf_model, input_dim):
    ...
    try:
        ...
        if rknn.load_onnx(model=onnx_path) != 0: return print("❌ Error cargar ONNX")
        
        # Importante: force_builtin_perm=True ayuda con capas LSTM en algunos casos
        if rknn.build(do_quantization=False) != 0: return print("❌ Error build RKNN")
        if rknn.export_rknn(rknn_path) != 0: return print("❌ Error export RKNN")
